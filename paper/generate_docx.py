...
abstract = (
    ...
    "将课程所学的NLP测度构建方法系统应用于一个具体管理问题，并提供了可复现的五步流水线代码与"
    "企业—年份面板数据集。需要说明，本文为课程预演稿，实证采用合成演示数据以跑通方法流程，"
    "真实研究需在获取巨潮资讯网与CSMAR数据后复现。"
)
...
da = (
    ...
    "标准差18.37，最小值0、最大值92.39，四分位距介于21.51与48.39之间，分布跨度大且右偏，"
    "说明样本公司在时间焦点上存在明显异质性：部分公司几乎不使用短期化表述，而部分公司的短视词频"
    "接近样本均值的近三倍；长期视域测度LongTerm均值为41.57，与短视测度量级相近。"
    "被解释变量RD_intensity（研发投入强度）均值为12.42%、标准差2.76，最小值3.31%、最大值18.84%，"
    "说明样本内研发投入水平存在充分变异。控制变量方面，公司规模（Size）均值21.96，"
    "资产收益率（ROA）均值3.9%，资产负债率（Lev）均值45.1%，上市年龄（Age）均值12.9年，"
    "营收成长性（Growth）均值12.0%，现金流比率（Cash）均值4.9%，各变量分布合理、无异常极端值。"
    "描述性统计确认了数据的基本合理性与变异充足性，为后续回归识别提供了基础。"
)
...
add_h2("4.4 分组对比与年度差异分析")
g1 = (
    "为进一步揭示测度的经济含义，本文以ShortTerm的样本中位数（32.52）将全部600个观测划分为高、"
    "低短视两组进行对比。高短视组（n=300）的短视测度均值为49.38，其研发投入强度均值仅为10.53%；"
    "低短视组（n=300）的短视测度均值为19.15，研发投入强度均值达14.31%。两组研发强度相差约3.77个"
    "百分点，相当于样本整体研发强度均值的三成左右，组间差距在未经任何回归调整的条件下已相当可观。"
    "组内相关性进一步支持这一判断：高短视组内部ShortTerm与RD_intensity的相关系数为-0.670，"
    "低短视组内部为-0.566，均保持显著负向；而按公司规模中位数划分的大小规模两组中，该相关系数分别"
    "为-0.838与-0.857，几乎不存在规模维度的异质性衰减。这说明短视与研发投入的负向关联并非由个别"
    "极端观测或特定规模区间驱动，而是在不同分组下普遍成立的横截面事实。"
)
add_para(g1, indent=True)
g2 = (
    "分年度看，2019—2023年各年ShortTerm均值依次为34.91、32.54、35.35、34.35与34.16，年度间波动"
    "较小，年度变量与ShortTerm的相关系数仅为0.002，表明短视水平更多体现为公司之间相对稳定的个体差异，"
    "而非随时间共同漂移的宏观现象。这一特征与测度的构建逻辑相吻合：管理者的时间偏好属于较为稳定的认知"
    "特质，其在披露文本中的语言痕迹因此具备跨期可比性，为面板固定效应识别提供了合理的设定基础。同时，"
    "各年RD_intensity均值稳定于12.2%—12.6%的窄幅区间，未呈现明显的共同趋势，排除了宏观研发周期对"
    "横截面比较的干扰。"
)
add_para(g2, indent=True)
g3 = (
    "上述结果可与经典理论相互印证。在Stein（1989）的两期模型中，当资本市场以当期盈利作为定价锚时，"
    "管理者存在牺牲长期价值以推高短期业绩的动机，回报周期长、当期费用化的研发支出首当其冲；本文以文本"
    "测度直接刻画了这一时间偏好的语言痕迹，并在回归与分组层面一致观察到其与研发投入的稳健负相关。"
    "从组间离散程度看，高短视组研发强度的标准差（2.24）高于低短视组（1.77），提示短视水平较高的公司"
    "在研发决策上分歧更大，与其更易受短期业绩压力扰动的行为逻辑一致。政策层面，这一发现支持将MD&A"
    "语言特征纳入信息披露质量监测的构想：对投资者而言，年报语言的短期化倾向可作为识别企业创新意愿衰减"
    "的早期信号；对监管与公司治理而言，优化高管考核周期、弱化短期业绩权重，有助于从源头上缓解语言与"
    "行为上的双重短视。当然，分组证据仅刻画条件均值差异，不构成因果识别；因果层面的确认仍有赖于真实"
    "数据下的准自然实验或工具变量设计。"
)
add_para(g3, indent=True)
...
rb = (
    ...
    "此外，4.4节中高、低短视组的组内相关（-0.670与-0.566）与全样本相关（-0.824）方向一致，"
    "亦旁证结果对样本切分方式不敏感。"
...
refs = [
    ...
    "曾庆生, 周波, 张程, 陈信元. 年报语调与内部人交易：“表里如一”还是“口是心非”？[J]. 管理世界, 2018(09): 143-160.",
    "沈艳, 陈赟, 黄卓. 文本大数据分析在经济学和金融学中的应用：一个文献综述[J]. 经济学(季刊), 2019, 18(04): 1473-1500.",
    "姚加权, 冯绪, 王赞钧, 纪荣嵘. 语调、情绪及市场影响：基于金融情绪词典[J]. 管理科学学报, 2021, 24(01): 26-46.",
    "汪昌云, 武佳薇. 媒体语气、投资者情绪与IPO定价[J]. 金融研究, 2015(09): 174-189.",
    "Li F. Annual report readability, current earnings, and earnings persistence[J]. "
    "Journal of Accounting and Economics, 2008, 45(2-3): 221-247.",
    "Brochet F, Loumioti M, Serafeim G. Speaking of the short-term: Disclosure horizon and managerial myopia[J]. "
    "Review of Accounting Studies, 2015, 20(3): 1122-1163.",
    "Bushee B J, Gow I D, Taylor D J. Linguistic complexity in firm disclosures: Obfuscation or novelty?[J]. "
    "Journal of Accounting Research, 2018, 56(1): 85-121.",
    "Antweiler W, Frank M Z. Is all that talk just noise? The information content of internet stock message boards[J]. "
